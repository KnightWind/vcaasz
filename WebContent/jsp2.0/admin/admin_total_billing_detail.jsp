...
<body>
<form id="query" name="query" action="/admin/billing/viewBillDetail" method="post">
...
<%--									<th class="name">${LANG['website.admin.bill.data.title.event'] }</th>--%>
								...
								<c:forEach var="videoBill" items="${confBill.videoBillings}" varStatus="statusV">
									<tr>
										<td class="first-child name">${videoBill.name }</td>
										<td class="type">Client</td>
<%--										<td class="number">  </td>--%>
<%--										<td class="name">--%>
<%--											<c:if test="${videoBill.billType eq 2 }">NameHost</c:if>--%>
<%--											<c:if test="${videoBill.billType eq 1 }">PerMin</c:if>--%>
<%--										</td>--%>
										<td class="start">
											<c:if test="${site.hireMode eq 1 }"><!-- 包月入会时间 是zoomConfRecord中的标准时间 -->
												<c:out value="${myfn:fmtDate('yyyy-MM-dd HH:mm',videoBill.startTime,site.timeZone)}"/>
											</c:if>
											<c:if test="${site.hireMode eq 2 }"><!-- 计时入会时间 是billing中的北京时间 -->
												<c:out value="${myfn:fmtDateByBJ('yyyy-MM-dd HH:mm',videoBill.startTime,site.timeZone)}"/>
											</c:if>
										</td>
								...
								<c:forEach var="tellBill" items="${confBill.telBillings}" varStatus="statusT">
									<tr>
										<td class="first-child name">${tellBill.name }</td>
										<td class="type">Tel</td>
<%--										<td class="number">${ tellBill.dnis}</td>--%>
<%--										<td class="name">Audio-Tel</td>--%>
										<td class="start"><c:out value="${myfn:fmtDateByBJ('yyyy-MM-dd HH:mm',tellBill.startTime,site.timeZone)}"/></td>
						...
						<c:if test="${fn:length(pageModel.datas) le 0}">
							<tr><td class="name" style="text-align: center;">${LANG['website.admin.bill.data.no'] }</td></tr>
						</c:if>
					</tbody>
				</table>
			</div>
...
function viewHostBillList(year,month){
	...
	window.location = "/admin/billing/listDetailBills?year="+year+"&month="+month;
}
