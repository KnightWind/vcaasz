				...
				<div class="box-wrapper box-wrapper-reset-pass">
					...
					<form id="pass-forgot-form" method="post" class="pass-forgot-form">
						...
						<div class="form-item email-widget">
							...
							<div class="widget">
								<input type="text" class="input-text" id="systemEmail" name="systemEmail"/>
								<p class="error">
<!-- 									<i class="icon"></i>请正确填写邮箱！ -->
									<span class="form-tip"><span id="systemEmailTip"></span></span>
								</p>
							</div>
						</div>
	...
	function sendEmail(){
		...
		app.forgotAdminPass(authCode, type, random, systemEmail, function(result) {
			...
			if (result && result.status==2) {
				$("#authCode").val("");
				$("#systemEmail").val("");
				successDialog(FORGET_CONSTANT.sendok.replaceAll("####userEmail####",systemEmail));	
			} else {
