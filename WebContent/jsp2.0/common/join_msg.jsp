...
<!-- 				<img src="/assets/images/bg/prompt.png" alt="prompt" /> -->
<%-- 				<c:set var="msgName" value="website.conf.join.type.${errorCode}"/>${LANG[msgName]} --%>
			</div>		
		</div>
	</div>
...
window.onload=function(){
	...
	<c:if test="${blank!=null && blank!='' && blank eq 1}">
		$(".biz-titleBar").css("display","none");
		$(".form-buttons").css("display","none");
	</c:if>
	<c:if test="${inviteflag!=null && inviteflag!='' && inviteflag eq 1}">
		$(".biz-titleBar").css("display","none");
	</c:if>
}
