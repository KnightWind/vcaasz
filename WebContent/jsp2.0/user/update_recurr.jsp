	...
	<div class="meeting-dialog">
		...
		<div class="step1-panel valid-group">
			<div class="step1-header">
				<h3 class="title">
				<i class="icon icon-meeting"></i><span class="confOperate">${LANG['website.user.conf.create.info.modifymeeting']}</span><!-- <span class="step fix-png"> -->
				</h3>
				<a href="javascript:closeDialog();" title="${LANG['website.common.option.close']}" class="close"></a>
			</div>
			<form id="scheduleForm1" method="post">
			<div class="step-content" style="padding-left: 10px;padding-right: 10px;">
...
<%--									<span class="form-tip"><span id="hostKeyTip"></span></span>--%>
			...
			<!-- 是否先于主持人入会（被禁止修改 --隐藏） 2014-10-29 恢复可修改 -->
			<div class="form-item">
			...
			<div class="form-item">
				...
				<div class="widget">
						<input type="radio" name="optionStartType"  value="1" <c:if test="${conf.optionStartType eq 1 }">checked="checked"</c:if>  /> ${LANG['user.menu.conf.pad.videooption.on'] }
						&nbsp;
						<input type="radio" name="optionStartType" value="2"  <c:if test="${conf.optionStartType eq 2 }">checked="checked"</c:if> /> ${LANG['user.menu.conf.pad.videooption.off'] }
...
<!-- 					 </select> -->
			...
			<div class="form-item">
				<label class="title">${LANG['website.user.conf.update.Cycle.conf'] }${LANG['website.common.symbol.colon'] }</label>
				<div class="widget">
					<input class="repeat-cycle" type="radio" name="repeat" id="repeatYes" value="1" checked="checked"  disabled="disabled"/>
					<label for="repeatYes">${LANG['website.common.item.yes']}</label>
					<input class="repeat-cycle" type="radio" name="repeat" id="repeatNot" value="2" disabled="disabled"/>
			...
			<div class="form-item">
				...
				<div class="widget">
					 <div style="background-color: #e6f5de; width: 376px; margin-left: 38px;">  &nbsp;${LANG['bizconf.jsp.conf.cycle.outlookGoogle.Setup']}</div>
				</div>
			</div>
			<!-- 会议描述 -->
	...
	function updatePMI(){
		...
		conf.optionStartType = $("input[name=optionStartType]:checked").val();
		$.ajax({
	      	type: "POST",
