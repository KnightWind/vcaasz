...
<div class="tabview">
	<div class="views">
		...
		<div class="summary">
			<ul class="actions">
				<li class=""><a onclick="return contactEdit();"><i class="icon icon-add"></i>${LANG['website.user.contact.list.addcontact']}</a></li>
				<li class=""><a onclick="return contactImport();"><i class="icon icon-bat"></i>${LANG['website.user.contact.list.batchadd']}</a></li>
				<c:if test="${fn:length(pageModel.datas) gt 0}">
					<li class=""><a onclick="return toExportContact('');"><i class="icon icon-exp"></i>${LANG['website.user.contact.list.exportall']}</a></li>
						...
						<c:forEach var="contact" items="${pageModel.datas}" varStatus="status">
							<tr>
								...
								<td class="last-child actions">
									<a class="modify" onclick="return contactEdit('${contact.id }')"><i class="icon icon-modify"></i>${LANG['website.common.option.modify']}</a>
...
function deleteContactBatch(){
	if($("input[name=id]:checked").length>0){
		parent.parent.promptDialog("${LANG['website.user.contact.list.deleteconfirm']}", function() {
				...
				$.ajax({
			        type: "POST",
			        url: "/user/contact/deleteContactBatch",
			        data: {id:temp},
			        traditional:true,//传递数组必须设置
			        success: function(data, textStatus, XmlHttpRequest) {
			        	parent.parent.successDialog("${LANG['website.user.contact.list.deleteconfirm.success.tip']}");
			        	window.location = "/user/contact/list";
			        },
