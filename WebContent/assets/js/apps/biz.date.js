...
Date.prototype.equalTo=function(date){
	if(this==null || date==null){
		return false;
	}
	var thisTimpStamp=this.getTime();
 ...
 *  日期加减方法
 */
Date.prototype.add = function(dateType, number) {
    var retDate = new Date(this.getTime());
    var BASEYEAR = 1970;
    var monthNum = 0;
    switch(dateType) {
        case DATETYPE_CONSTANT.YEAR:
            monthNum = number * 12;
        case DATETYPE_CONSTANT.MONTH:
            if (monthNum == 0) {
                monthNum = number;
            }
            var thisDay = retDate.getDate();
            var tarCountMonth = (retDate.getFullYear() - BASEYEAR) * 12 + retDate.getMonth() + monthNum;
            var tarYear = Math.floor(tarCountMonth / 12) + BASEYEAR;
            var tarMonth = tarCountMonth % 12;
            var tarMaxDay = getMonthMaxDay(tarYear, tarMonth + 1);
            retDate = new Date(tarYear, tarMonth, (thisDay > tarMaxDay ? tarMaxDay : thisDay));
            with (retDate) {
                setHours(this.getHours());
                setMinutes(this.getMinutes());
                setSeconds(this.getSeconds());
                setMilliseconds(this.getMilliseconds());
            }
            break;
        case DATETYPE_CONSTANT.WEEK:
            var thisDate = retDate.getDate();
            retDate.setDate(thisDate + number * 7);
            break;
        case DATETYPE_CONSTANT.DAY:
            var thisDate = retDate.getDate();
            retDate.setDate(thisDate + number);
            break;
        case DATETYPE_CONSTANT.HOUR:
            var thisHour = retDate.getHours();
            retDate.setHours(thisHour + number);
            break;
        case DATETYPE_CONSTANT.MINUTE:
            var thisMinute = retDate.getMinutes();
            retDate.setMinutes(thisMinute + number);
            break;
        case DATETYPE_CONSTANT.SECOND:
            var thisSecond = retDate.getSeconds();
            retDate.setSeconds(thisSecond + number);
            break;
        case DATETYPE_CONSTANT.MILLSECONDS:
            var thisMillSecond = retDate.getMilliseconds();
            retDate.setMilliseconds(thisMillSecond + number);
            break;
        default:
            break;
    }
...
Date.prototype.addYear = function(number) {
    return this.add(DATETYPE_CONSTANT.YEAR, number);
};
...
Date.prototype.addMonth = function(number) {
    return this.add(DATETYPE_CONSTANT.MONTH, number);
};
...
Date.prototype.addWeek = function(number) {
    return this.add(DATETYPE_CONSTANT.WEEK, number);
};
...
Date.prototype.addDay = function(number) {
    return this.add(DATETYPE_CONSTANT.DAY, number);
};
/**
 *小时加减
 */
Date.prototype.addHour = function(number) {
    return this.add(DATETYPE_CONSTANT.HOUR, number);
};
/**
 *分加减
 */
Date.prototype.addMinute = function(number) {
    return this.add(DATETYPE_CONSTANT.MINUTE, number);
};
/**
 *秒加减
 */
Date.prototype.addSecond = function(number) {
    return this.add(DATETYPE_CONSTANT.SECOND, number);
};
/**
 *毫秒加减
 */
Date.prototype.addMillSecond = function(number) {
    return this.add(DATETYPE_CONSTANT.MILLSECONDS, number);
};
